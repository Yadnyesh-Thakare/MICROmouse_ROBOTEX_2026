...
const bool useRightHandRule = true;
// ================================================================
...
void uTurn();
void driveStep();
void leftHandRule(bool lWall, bool rWall);
void rightHandRule(bool lWall, bool rWall);
...
// ================================================================
//  TURN HIGH-LEVEL WRAPPERS
// ================================================================
void turnLeft() {
  stopMotors(); delay(100);
  spotTurn(90.0f, false); // Counter-clockwise 90°
  correctWallPosition();
  blinkLED(LED_LEFT, 1);
}
...
void turnRight() {
  stopMotors(); delay(100);
  spotTurn(90.0f, true);  // Clockwise 90°
  correctWallPosition();
  blinkLED(LED_RIGHT, 1);
}
...
// ================================================================
//  DRIVE STEP — continuous wall-following PID
// ================================================================
void driveStep() {
  ...
  if (useLeftHandRule) {
    if (hasLeft) {
      error = (float)(distLeft - IDEAL_SIDE_DIST);  wallFound = true;
    } else if (hasRight) {
      error = -(float)(distRight - IDEAL_SIDE_DIST); wallFound = true;
    }
  } else {
    if (hasRight) {
      error = -(float)(distRight - IDEAL_SIDE_DIST); wallFound = true;
...
// ================================================================
void leftHandRule(bool lWall, bool rWall) {
  if (!lWall)      { Serial.println("Turn LEFT");  turnLeft();  }
  else if (!rWall) { Serial.println("Turn RIGHT"); turnRight(); }
  else             { Serial.println("U-TURN");     uTurn();     }
}

void rightHandRule(bool lWall, bool rWall) {
  if (!rWall)      { Serial.println("Turn RIGHT"); turnRight(); }
  else if (!lWall) { Serial.println("Turn LEFT");  turnLeft();  }
  else             { Serial.println("U-TURN");     uTurn();     }
}
// ================================================================
//  MAIN LOOP
...
void loop() {
  ...
  if (distCenter <= FRONT_STOP_DIST) {
    ...
    bool lWall = (distLeft  < SIDE_WALL_DETECT);
    bool rWall = (distRight < SIDE_WALL_DETECT);

    Serial.print(useRightHandRule ? "LEFT  | " : "RIGHT | ");
    Serial.print("C:"); Serial.print(distCenter);
    Serial.print(" L:"); Serial.print(distLeft);
    Serial.print(" R:"); Serial.print(distRight);
    Serial.print(" | ");
    ...
    if (useRightHandRule) {
        rightHandRule(lWall, rWall);
    }
    else {leftHandRule(lWall, rWall);
    }
  } else {
    distLeft  = readSensor(sensorLeft,  999);
    distRight = readSensor(sensorRight, 999);
